...
export function useFilter() {
  ...
  return { filterKirim, filterKas, filterProduksi, filterKirimDateStart, filterKirimDateEnd, filterKasDateStart, filterKasDateEnd, filterProduksiDateStart, filterProduksiDateEnd, setFilter, getPredicate };
}

export function useForm(data, konsinyasiLama, totalTagihan, saldoPelanggan, showToast) {
  const form = reactive({ pelanggan_id: '', kirim_baru: [], bayar: 0, gunakanSaldo: false, bayarSekarang: true, simpanSaldo: false });
  ...
  const closeModal = () => { modalOpen.value = false; isSaving.value = false; };
  ...
  return { form, returInput, modalOpen, isSaving, errorMsg, openModal, closeModal, addKirimBaru, submit };
        }
