...
export const Query = {
  async all() {
    const [pelanggan, barang, konsinyasi, kas, pembayaran, produksi, stok_log, saldo_log] = await Promise.all([
      db.pelanggan.toArray(), db.barang.toArray(), db.konsinyasi.toArray(), db.kas.toArray(),
      db.pembayaran.toArray(), db.produksi.toArray(), db.stok_log.toArray(), db.saldo_log.toArray()
    ]);
...
export const Command = {
  ...
  async reset() { await db.delete(); await db.open(); }
};
